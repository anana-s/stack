...
template <typename istream> class lex {
...
lex<istream>::lex(istream &&source) noexcept
    : source(std::move(source)){
          // noop
      };

template <typename istream> token lex<istream>::get() {
  if (buffer.empty() && !(source >> buffer)) {
    return token::eof();
  }

  if (std::smatch result; std::regex_search(
          buffer, result, std::regex("^\\s*([a-zA-Z_][a-zA-Z0-9_]*)"))) {
...
lex<istream>::iterator::iterator() noexcept
    : lexeme(nullptr), pos(std::numeric_limits<size_t>::max()),
      cur(token::eof()){
          // noop
      };
